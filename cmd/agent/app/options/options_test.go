...
func TestAgentOptions_Validate(t *testing.T) {
	tests := []struct {
		...
		{
			name: "both enable-tunnel and enable-proxy are false",
			options: &AgentOptions{
				TunnelOptions: TunnelOptions{
					VPNDriver: libreswan.DriverName,
					MACPrefix: "aa:0f",
			...
			wantErr: true,
			errMsg:  "at least one of --enable-tunnel or --enable-proxy must be true",
		},
		{
			name: "only enable-tunnel",
			options: &AgentOptions{
				TunnelOptions: TunnelOptions{
					VPNDriver: libreswan.DriverName,
					MACPrefix: "aa:0f",
...
func TestNewDefaultOptions(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !got.EnableTunnel {
				t.Errorf("NewDefaultOptions().EnableTunnel = false, want true")
			}
			if !got.EnableProxy {
				t.Errorf("NewDefaultOptions().EnableProxy = false, want true")
			}
		})
	}
}
